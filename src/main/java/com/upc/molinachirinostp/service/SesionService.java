...
public class SesionService {
    ...
    public SesionViewDTO crear(CrearSesionDTO dto) {
        ...
        // Buscar o crear alumno automáticamente
        Alumno a = alumnoRepo.findByUsuario_IdUser(dto.idUsuario())
                .orElseGet(() -> {
                    // Crear alumno automáticamente si no existe
                    ...
                    nuevoAlumno.setSemestre("N/A");
                    nuevoAlumno.setEstatusEstudio("ACTIVO");
    ...
    public List<SesionViewDTO> porUsuarioDesdefecha(Long idUsuario, LocalDate fechaDesde) {
        // Buscar el alumno por usuario
        Alumno alumno = alumnoRepo.findByUsuario_IdUser(idUsuario)
                .orElseThrow(() -> new IllegalArgumentException("No se encontró alumno para el usuario"));
        
        // Buscar sesiones desde la fecha en adelante
        return sesionRepo.findByAlumnoIdAlumnoAndFechaInicioGreaterThanEqual(alumno.getIdAlumno(), fechaDesde)
                .stream()
                .map(this::toView)
                .toList();
    }
